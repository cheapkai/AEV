import scipy.io
import numpy as np
import sys

def switch_demo(argument):
    switcher = {
        0: 0,
        1: 1,
        6: 2,
        7: 3,
        8: 4,
        
    }
    return switcher.get(argument, 5)


def cutoffradial(Rij) :
	if (Rij > 3.9) :
		return 0.0

	return (0.5*cos((3.14*Rij)/Rc) + 0.5)	

def cutoffangular(Rij) :
	if (Rij > 4.1) :
		return 0.0

	return (0.5*cos((3.14*Rij)/Rc) + 0.5)


def radial(Ri,Rj,Rs,eta) :
	Rij = sqrt((Ri[0]-Rj[0])*(Ri[0]-Rj[0]) + (Ri[1]-Rj[1])*(Ri[1]-Rj[1]) + (Ri[2]-Rj[2])*(Ri[2]-Rj[2]))

	radf = exp((((-1)*(Rij-3.9)*(Rij-3.9))))*cutoffradial(Rij)

	return (radf)	

		
def angular(Ri,Rj,Rk,Rs,eta,zeta,theta) :
	Vj = Rj - Ri
	Vk = Rk - Ri
	angle = V]




	




qm7 = scipy.io.loadmat('/home/mehthab/Downloads/qm7.mat')

#print(str(qm7))

X = np.array(qm7['X'])
R = np.array(qm7['R'])
T = np.array(qm7['T'])
P = np.array(qm7['P'])
Z = np.array(qm7['Z'])

#X is Coulomb Matriz
#T is Atomization Energies - labels
#P is splits for cross-validation
#Z is atomic charge of each atom in the molecule
#R is cartesian coordinates of each atom in the molecule

xS = np.shape(X)
rS = np.shape(R)
tS = np.shape(T)
zS = np.shape(Z)
pS = np.shape(P)

print(str(rS))



#sys.exit()
#Calculating stuff from given matrices

print(str(Z[2700]))

#Stuff we need 
Rs = [0.50,1.17,1.83,2.50,3.17,3.83,4.50,5.17]
eta = 4.00
zeta = 8.00
the = [0.00,1.57,3.14,4.71]
Ar = np.zeros([23,520,7165])

rc = np.zeros([7165,23])

a1 = [0,8,16,24,32]
a2 = np.zeros([5,5])

a2[0][0] = 40
a2[0][1] = 72
a2[0][2] = 104
a2[0][3] = 136
a2[0][4] = 168
a2[1][1] = 200
a2[1][2] = 232
a2[1][3] = 264
a2[1][4] = 296
a2[2][2] = 328
a2[2][3] = 360
a2[2][4] = 392
a2[3][3] = 424
a2[3][4] = 456
a2[4][4] = 488

a2[0][0] = 40
a2[1][0] = 72
a2[2][0] = 104
a2[3][0] = 136
a2[4][0] = 168
a2[1][1] = 200
a2[2][1] = 232
a2[3][1] = 264
a2[4][1] = 296
a2[2][2] = 328
a2[3][2] = 360
a2[4][2] = 392
a2[3][3] = 424
a2[4][3] = 456
a2[4][4] = 488


for i in range(71) :
	for j in range(23) :
		u = int(Z[i][j])
		v = switch_demo(u)
		rc[i][j] = v
		rc[i][j] = int(rc[i][j])

for k in range(7165) :
	for i in range(23) :

		if(int(rc[k][i])==0) :
			continue

		for j in range(23) :
			if (i==j) :
				continue

			if (int(rc[k][j]) == 0) :
				continue

			start = a1[int(rc[k][j])-1]

			for y in range(8) :

				#Ar[i][start + y][k] = Ar[i][start + y][k] + fc1(R[k][i],R[k][j],Rs[y],eta)
				Ar[i][start + y][k] = Ar[i][start + y][k] + 0

			for y2 in  range((j+1),23):

				if (y2==i) :
					continue

				if (y2==j) :
					continue

				if (int(rc[k][y2]) == 0) :
					continue

				start = a2[int(rc[k][j]-1)][int(rc[k][y2]-1)]

				for y3 in range(8) :

					for y4 in range(4) :

						#Ar[i][start + ((y3+1)*(y4+1) - 1)][k] = Ar[i][start + ((y3+1)*(y4+1) - 1)][k] + fc2(R[k][i],R[k][j],R[k][y2],i,Rs[y3],eta,the[y4],zeta)
						Ar[i][int(start + ((y3+1)*(y4+1) - 1))][k] = Ar[i][int(start + int((y3+1)*(y4+1) - 1))][k] + 0



scipy.io.savemat('feature_vector.mat', {'AEVs' : Ar , 'Atomic_Num' : rc , 'labels' : T})

#return Ar,rc

				

				



				



				

